...
async def embed_images_batch_async(image_paths: List[Path], batch_size: int = 12) -> List[List[float]]:
    """
    Batch embed images using Jina CLIP-v2.
    Processes in batches to avoid API limits.
    """
...
async def embed_text_async(text: str) -> List[float]:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        response = await client.post(
            ...
            json={"model": MODEL_NAME, "input": [{"text": text}]}
        )
        response.raise_for_status()
        return response.json()["data"][0]["embedding"]
...
async def embed_image_bytes_async(image_bytes: bytes) -> List[float]:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        response = await client.post(
            ...
            json={"model": MODEL_NAME, "input": [{"image": image_b64}]}
        )
        response.raise_for_status()
        return response.json()["data"][0]["embedding"]
